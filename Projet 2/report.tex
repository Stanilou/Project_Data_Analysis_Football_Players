...
\begin{document}
    ...
    \section{Résultats et interprétation}
        L’analyse des différents graphiques réalisés nous a permis de dégager plusieurs tendances claires et d’identifier des éléments importants concernant l’impact de la musique sur la santé mentale :\\

        \begin{itemize}
            \item Dans la grande majorité des cas la musique (75\%) a un effet positif sur la santé mentale des auditeurs\\
            \item Dans des très rares cas (2,4\%) la musique a un effet négatif sur la santé mentale des auditeurs\\
            \item Les genres musicaux les plus bénéfiques sont le gospel et le lofi. Suivi de près par le hip-hop et la country\\
            \item Les heures d'écoutes quotidiennes n'ont pas d'impact significatif sur l'effet de la musique\\
            \item On voit qu'en terme de troubles mentaux, les plus atténués par la musique sont l'anxiété et la dépression\\
            \item Mais que les plus exacerbés par la musique est également la dépression et l'anxiété\\
            \item On peut déterminer que plus on est jeune plus la musique a un effet bénéfique sur la santé mentale, mais elle reste utile dans minimum 60\% des cas dans tout les âges\\
            \item Si jamais une personne est compositrice ou pratique un instrument, la musique est plus susceptible d'avoir un effet bénéfique sur sa santé mentale
        \end{itemize}
    ...
    \section{Conclusion}
        En conclusion, notre analyse a mis en évidence une relation significative entre les préférences musicales et la santé mentale des individus. La musique apparaît comme un outil potentiellement bénéfique pour le bien-être mental, avec des variations selon les genres musicaux et les caractéristiques des auditeurs. Cependant, les limites du jeu de données et du modèle utilisé soulignent la nécessité de poursuivre les recherches dans ce domaine. Des études complémentaires, intégrant des approches plus diversifiées et des données plus robustes, pourraient approfondir notre compréhension de l’impact de la musique sur la santé mentale.
